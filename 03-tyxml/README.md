...
accepts things like ``Hidden`, ``Text`, ``Password`.  `a_button_type`
accepts things like ``Submit`, and `a_method` (for forms), ``Post` and
other form submition keyword.
...
generated by TyXML. A simpler layout has been used instead
(table/tr/td/form), but it can't place each input field in a separate
column.
